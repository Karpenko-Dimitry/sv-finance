...
class PaymentInvoiceFormController extends Controller
{
    ...
    {
        return view('forms.payment-invoices');
    }

    /**
     * @param StorePaymentInvoiceRequest $request
     * @return JsonResponse
    ...
    public function store(StorePaymentInvoiceRequest $request)
    {
        ...
        $paymentInvoiceAction = new PaymentInvoices();
        $order->syncSteps(
            current_key: $paymentInvoiceAction->getActionKey('cart'),
            name: trans('telegram.payment_invoice.order.form'),
            form_data: $formData,
